...
export function articleLink( data, headTag = 'h2', loading = 'eager' ) {
  ...
  return `
    <article${ data.featured ? ' class="featured"' : '' }>
      <a href="${ data.link }">
        ${ data.imageThumb ? `<img src="${ data.imageThumb }"${ data.imageAlt ? ` alt="${ data.imageAlt }"` : '' } loading="${ loading }">` : '<svg><use xlink:href="#uraniumlogo"></use></svg>' }

        <p class="topic">${ data.topic }</p>

        <${ headTag }>${ title( data.title ) }</${ headTag }>
